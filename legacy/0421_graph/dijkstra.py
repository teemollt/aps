...
# 2. 새로운 경로가 생길때마다, 해당경로를 이용하는 비용계산
...
def dijkstra(start, adj, weight):
    ...
    U = {start}
    # 모든 노드들이 선택될때까지 아래 사항을 반복
    # 1. 현재 선택된 노드들을 통해서, 갈수 있는 경로 중 최소 비용을 가지는 경로를 선택
    ...
    #최소 비용을 가지는 노드를 찾았음
    U.add(min_idx)
    # 지금 선택된 노드를 통해서 다른 노드들로 갈 수 있는 경로의 비용을 업데이트
    for i in range(1, V+1):
        if i not in U:
            # 아직 선택되지 않은 노드들의 경로 비용을 다시 계산
            # min_idx 에서 i번 노드로 가는 비용 + 시작에서 min_idx로 가는 비용
...
#시작정점 : 1
start = 1
# 시작정점에서 다른 정점들까지 가는데 드는 비용(가중치) 배열