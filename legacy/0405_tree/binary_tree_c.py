class MyBinaryTree:
    ...
    def insert(self, parent, child):
        # 만약에 tree 안에 부모가 없으면 => root
        # parent는 root, child 왼쪽자식
        if parent not in self.tree:
            self.tree[1] = parent
            self.tree[2] = child
        else:
            # 부모가 tree 안에 있으면,
            # child를 tree에 추가 해야하는데
            # child의 index를 알아야함 >> 부모의 인덱스를 확인
    ...
    # 전위 순회
    def preorder(self, v):
        # 좌 우 서브트리를 순회하기 전에, 현재 노드에서 작업을 먼저 수행
        # v가 유효한
        if self.tree[v] != 0:
...
data = list(map(int, "1 2 1 3 2 4 3 5 3 6 4 7 5 8 5 9 6 10 6 11 7 12 11 13"))
H = 4
